...
export const CliLoginView = ({ data, status, actions }: CliLoginViewProps): JSX.Element => (
        ...
        <div class='flex flex-col gap-5'>
          ...
          <div class='flex flex-col gap-3'>
            ...
            <p class='min-h-5 text-sm text-text-secondary' aria-live='polite'>
              {status.copy.value === 'copied' && 'Copied'}
              {status.copy.value === 'failed' && 'Copy failed. Select the code manually.'}
            </p>
          </div>
        </div>
      ) : (
