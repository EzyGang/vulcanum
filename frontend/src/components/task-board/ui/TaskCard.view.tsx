...
export const TaskCard = ({ data }: TaskCardProps): JSX.Element => (
  <article
    ...
    class='relative flex cursor-pointer flex-col gap-3 rounded-none border border-border-base bg-bg-input p-4 text-left transition-colors hover:border-border-focus focus-visible:border-border-focus focus-visible:outline-none'
  >
    ...
    <div class='flex flex-wrap gap-1'>
      {data.task.labels.map((label) => (
        <span
          key={label.id}
          class='flex items-center gap-1 border border-border-base bg-bg-card px-2 py-0.5 text-xs font-medium uppercase tracking-wide text-text-secondary'
        >
          <span
    ...
    {data.menuOpen && (
      <div
        role='menu'
        class='fixed z-50 flex max-h-72 w-56 flex-col overflow-y-auto rounded-none border border-border-base bg-bg-card p-1 shadow-modal'
        style={data.menuStyle}
