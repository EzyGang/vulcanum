...
impl WorkRunsRepository {
        ...
        Q: Queryer<'c>,
    {
        ...
        let positions = (0..pr_urls.len() as i32).collect::<Vec<i32>>();

        QueryBuilder::<Postgres>::new("INSERT INTO work_run_prs (work_run_id, pr_url, position) ")
            .push_values(
                pr_urls.iter().zip(positions.iter()),
                |mut b, (pr_url, position)| {
                    b.push_bind(work_run_id)
                        .push_bind(pr_url)
                        .push_bind(position);
                },
            )
            .push(" ON CONFLICT (work_run_id, pr_url) DO UPDATE SET position = EXCLUDED.position")
            .build()
            .execute(db)
            .await
            .map_err(WorkRunsError::from)?;
