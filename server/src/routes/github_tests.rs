...
async fn webhook_accepts_valid_delivery(pool: sqlx::PgPool) {
    ...
    let request = test::TestRequest::post()
        ...
        .insert_header((
            "X-Hub-Signature-256",
            test_helpers::github::sign_github_webhook(&payload),
        ))
...
#[sqlx::test]
async fn webhook_returns_service_unavailable_when_queue_is_unavailable(pool: sqlx::PgPool) {
    let mut state = test_helpers::state::build_state(pool.clone()).await;
    state.github_webhooks = GithubWebhookService::new(
        Some(Arc::from(test_helpers::GITHUB_WEBHOOK_SECRET)),
        Some(Arc::from("vulcanum-app")),
        state.is_single_user,
        GithubWebhookStore::redis("redis://127.0.0.1:1").expect("create Redis client"),
        state.jobs.clone(),
        Arc::new(state.github.clone()),
    );
    let app = test::init_service(
    ...
    let request = test::TestRequest::post()
        ...
        .insert_header(("X-GitHub-Event", "pull_request"))
        .insert_header(("X-GitHub-Delivery", "delivery-queue-unavailable"))
        .set_payload(payload)
        .to_request();
...
async fn webhook_rejects_invalid_signature(pool: sqlx::PgPool) {
    ...
    let request = test::TestRequest::post()
        ...
        .set_payload(test_helpers::github::github_webhook_payload("closed"))
        .to_request();
