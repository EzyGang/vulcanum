...
#[sqlx::test]
async fn rename_worker_returns_updated_worker(pool: sqlx::PgPool) {
    ...
    let req = test::TestRequest::patch()
        ...
        .insert_header(auth_header(&token))
        .set_json(serde_json::json!({ "name": "meaningful-name" }))
        .to_request();
    let resp = test::call_service(&app, req).await;

    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = test::read_body_json(resp).await;
    assert_eq!(body["id"], worker_id.to_string());
    assert_eq!(body["name"], "meaningful-name");
}
