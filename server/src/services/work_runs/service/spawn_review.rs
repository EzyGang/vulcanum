...
impl WorkRunsService {
    pub(crate) async fn attach_prs_and_spawn_reviews(
        ...
        let task_data = match self
            .fetch_task_render_data(&project_config, &run.external_task_ref)
            .await
        {
        ...
        self.update_task_pr_block(run, &task_prs, &task_data.body)
            .await;
    ...
    async fn update_task_pr_block(&self, run: &WorkRun, task_prs: &[TaskPr], task_body: &str) {
        let project_config = match self.project_configs.find_by_id(run.project_config_id).await {
            Ok(config) => config,
            Err(_) => return,
        };
