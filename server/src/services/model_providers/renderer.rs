...
impl RenderedModelConfig {
    ...
    pub fn from_connected(
                ...
                for (key, value) in credentials {
                    match value.as_str() {
                        Some(secret) if !secret.is_empty() => {
                            let env_key = Self::credential_env_key(key);
                            env.insert(env_key.clone(), secret.to_owned());
                            options
                                .insert("apiKey".to_owned(), json!(format!("{{env:{env_key}}}")));
                        }
                        Some(_) => (),
                        None => tracing::warn!(
        ...
        let primary = Self::model_ref(selection.primary_provider_key, selection.primary_model_id);
        let small = Self::model_ref(selection.small_provider_key, selection.small_model_id);

        let mut root = serde_json::Map::new();
        root.insert("permission".to_owned(), Self::permission_config());
    ...
    fn credential_env_key(key: &str) -> String {
        Self::decode_legacy_snake_case_env_key(key).unwrap_or_else(|| key.to_owned())
    }
