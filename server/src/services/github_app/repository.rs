...
impl GithubAppRepository {
    ...
    pub async fn get_installation<'c, Q: Queryer<'c>>(
        ...
        let row = sqlx::query_as::<_, GithubInstallation>(
            "SELECT id, account_login, created_at FROM github_installations LIMIT 1",
        )
        .fetch_optional(db)
        .await
    ...
    pub async fn insert_installation<'c, Q: Queryer<'c>>(
        ...
        let row: GithubInstallation =
            sqlx::query_as("INSERT INTO github_installations (account_login) VALUES ($1) RETURNING id, account_login, created_at")
                .bind(account_login)
                .fetch_one(db)
                .await
                .map_err(GithubAppError::Database)?;
    ...
    pub async fn delete_installation<'c, Q: Queryer<'c>>(
        ...
        let rows = sqlx::query("DELETE FROM github_installations WHERE id = $1")
            .bind(id)
            .execute(db)
            .await
            .map_err(GithubAppError::Database)?
