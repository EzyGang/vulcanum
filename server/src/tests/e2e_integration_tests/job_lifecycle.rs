...
async fn full_job_lifecycle(pool: sqlx::PgPool) {
    ...
    let project_id =
        test_helpers::project_configs::insert_project_config(&pool, "kaneo-lifecycle").await;
    let wr_id =
        test_helpers::work_runs::insert_pending_work_run(&pool, project_id, "task-lifecycle").await;
...
async fn ack_wrong_worker_returns_409(pool: sqlx::PgPool) {
    ...
    let project_id =
        test_helpers::project_configs::insert_project_config(&pool, "kaneo-wrong-ack").await;
    let wr_id =
        test_helpers::work_runs::insert_pending_work_run(&pool, project_id, "task-wrong-ack").await;
...
async fn double_ack_returns_409(pool: sqlx::PgPool) {
    ...
    let project_id =
        test_helpers::project_configs::insert_project_config(&pool, "kaneo-double-ack").await;
    let wr_id =
        test_helpers::work_runs::insert_pending_work_run(&pool, project_id, "task-double-ack")
            .await;
