...
use vulcanum_shared::api_types::{SubmitResultRequest, SubmitReviewResult};
...
pub(crate) struct FailedResult {
    ...
    pub(crate) review_result: Option<SubmitReviewResult>,
}
...
pub(crate) async fn submit_turn_result(
    ...
    let result = submit_result_request(SubmitResultParams {
        ...
        review_result: finish_artifact.map(|a| SubmitReviewResult {
            review_url: a.review_url.clone(),
            review_body: a.review_body.clone(),
            review_already_exists: a.review_already_exists,
        }),
    });
