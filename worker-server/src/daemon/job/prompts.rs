...
or `failed` when the task cannot be completed. Use `completed` only after running the formatter, validation, \
and test commands required for every repository you changed, or after recording why a required command could not run. \
...
or failing formatter, validation, or test commands."
        }
    }
}
