...
pub(super) fn journal_entry_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<JournalEntry> {
    ...
    Ok(JournalEntry {
        ...
        max_turns: row.get(23)?,
        host_pid: row.get(24)?,
        host_port: row.get(25)?,
        agent_backend: row.get(26)?,
        agent_session_path: row.get(27)?,
        agent_config_dir: row.get(28)?,
        agent_state_dir: row.get(29)?,
        agent_transport: row.get(30)?,
        agent_pid: row.get(31)?,
    })
}
